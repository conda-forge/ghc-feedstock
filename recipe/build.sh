...
pushd binary
  ...
  LDFLAGS="-L$BUILD_PREFIX/lib -Wl,-rpath,$BUILD_PREFIX/lib -Wl,-rpath-link,$BUILD_PREFIX/lib" \
  CC=$BUILD-gcc \
  AR=$BUILD-ar \
  LD=$BUILD-ld \
  NM=$BUILD-nm \
...
pushd source
  ...
  make show VALUE=libraries/binary_dist-boot_v_ALL_OBJS
  make show VALUE=$PREFIX/libraries/binary_dist-boot_v_ALL_OBJS
