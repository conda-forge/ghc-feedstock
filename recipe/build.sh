...
./configure --prefix=$PREFIX --enable-bootstrap-with-devel-snapshot --with-ffi-includes=$PREFIX/include --with-ffi-libraries=$PREFIX/lib --with-system-libffi AR=x86_64-conda_cos6-linux-gnu-ar CPP=cpp CPPFLAGS=-I$PREFIX/include --with-gmp-includes=$PREFIX/include --with-curses-libraries=$PREFIX/lib --with-gmp-libraries=$PREFIX/lib LDFLAGS=-L$PREFIX/lib CC=$BUILD_PREFIX/x86_64-conda_cos6-linux-gnu/bin/cc LD=$BUILD_PREFIX/x86_64-conda_cos6-linux-gnu/bin/ld.gold LIBTOOL=LD=$BUILD_PREFIX/x86_64-conda_cos6-linux-gnu/bin/libtool AR=$BUILD_PREFIX/x86_64-conda_cos6-linux-gnu/bin/ar CFLAGS=-fuse-ld=gold CONF_GCC_LINKER_OPTS_STAGE1=-fuse-ld=gold CONF_GCC_LINKER_OPTS_STAGE2=-fuse-ld=gold
#cat mk/config.mk
