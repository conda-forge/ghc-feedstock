...
# Generated by GNU Autoconf 2.71 for GHC run-time system 1.0.2.
#
...
PACKAGE_NAME='GHC run-time system'
PACKAGE_TARNAME='rts'
PACKAGE_VERSION='1.0.2'
PACKAGE_STRING='GHC run-time system 1.0.2'
PACKAGE_BUGREPORT='libraries@haskell.org'
PACKAGE_URL=''

ac_unique_file="include/rts/Constants.h"
...
enable_asserts_all_ways
enable_largefile
enable_large_address_space
'
...
if test "$ac_init_help" = "long"; then
  ...
  cat <<_ACEOF
\`configure' configures GHC run-time system 1.0.2 to adapt to many kinds of systems.
...
Fine tuning of the installation directories:
  ...
  --docdir=DIR            documentation root [DATAROOTDIR/doc/rts]
...
if test -n "$ac_init_help"; then
  case $ac_init_help in
     short | recursive ) echo "Configuration of GHC run-time system 1.0.2:";;
   esac
  cat <<\_ACEOF
...
Optional Features:
  ...
  --enable-large-address-space
                          Use a single large address space on 64 bit systems
...
if $ac_init_version; then
  cat <<\_ACEOF
GHC run-time system configure 1.0.2
...
It was created by GHC run-time system $as_me 1.0.2, which was
...
as_fn_append ac_func_c_list " pthread_condattr_setclock HAVE_PTHREAD_CONDATTR_SETCLOCK"
...
ac_config_files="$ac_config_files ghcplatform.h.top"


ac_config_headers="$ac_config_headers ghcautoconf.h.autoconf"


# Check whether --enable-asserts-all-ways was given.
if test ${enable_asserts_all_ways+y}
then :
  enableval=$enable_asserts_all_ways; case x"$enableval" in #(
  x"yes") :
    EnableAssertsAllWays=YES ;; #(
  x"no") :
    EnableAssertsAllWays=NO ;; #(
  *) :
    as_fn_error $? "invalid value: \"$enableval\" is not \"yes\" or \"no\"" "$LINENO" 5 ;;
esac
...
printf "%s\n" "#define USE_ASSERTS_ALL_WAYS 1" >>confdefs.h
...
if test "$CABAL_FLAG_tables_next_to_code" = 1
then :

printf "%s\n" "#define TABLES_NEXT_TO_CODE 1" >>confdefs.h
...
then :
  printf "%s\n" "#define _XOPEN_SOURCE 500" >>confdefs.h
    ...
    { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether C compiler supports __atomic_ builtins" >&5
printf %s "checking whether C compiler supports __atomic_ builtins... " >&6; }
    cat confdefs.h - <<_ACEOF >conftest.$ac_ext
/* end confdefs.h.  */
int x, y;
int
main (void)
{
__atomic_load(&x, &y, __ATOMIC_SEQ_CST); return y
  ;
  return 0;
}
_ACEOF
...
printf "%s\n" "yes" >&6; }
        ...
        need_latomic=0
        { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether -latomic is needed for sub-word-sized atomic operations" >&5
printf %s "checking whether -latomic is needed for sub-word-sized atomic operations... " >&6; }
        cat confdefs.h - <<_ACEOF >conftest.$ac_ext
/* end confdefs.h.  */
unsigned char a;
int
...
printf "%s\n" "failed" >&6; }
                as_fn_error $? "sub-word-sized atomic operations are not available." "$LINENO" 5
...
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    conftest$ac_exeext conftest.$ac_ext
        { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether -latomic is needed for 64-bit atomic operations" >&5
printf %s "checking whether -latomic is needed for 64-bit atomic operations... " >&6; }
        cat confdefs.h - <<_ACEOF >conftest.$ac_ext
/* end confdefs.h.  */
...
printf "%s\n" "yes" >&6; }
                ...
                { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: failed" >&5
printf "%s\n" "failed" >&6; }
                as_fn_error $? "64-bit atomic operations are not available." "$LINENO" 5
...
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    conftest$ac_exeext conftest.$ac_ext
        ...
        { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: no" >&5
printf "%s\n" "no" >&6; }
        as_fn_error $? "C compiler needs to support __atomic primitives." "$LINENO" 5

fi
rm -f core conftest.err conftest.$ac_objext conftest.beam conftest.$ac_ext


printf "%s\n" "#define NEED_ATOMIC_LIB $need_latomic" >>confdefs.h



   { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether C compiler has an LLVM back end" >&5
printf %s "checking whether C compiler has an LLVM back end... " >&6; }
...
if test x"$CcLlvmBackend" = x"YES"
then :

printf "%s\n" "#define CC_LLVM_BACKEND 1" >>confdefs.h
  ...
  *)
    ...
    case "$build_os" in
      ...
      freebsd|dragonfly|hpux|linuxaout|kfreebsdgnu|freebsd2|darwin|osx|nextstep2|nextstep3|sunos4|ultrix|haiku)
        ...
        ;;
      msys)
        as_fn_error $? "Building GHC using the msys toolchain is not supported; please use mingw instead. Perhaps you need to set 'MSYSTEM=CLANG64?'" "$LINENO" 5
        ;;
    ...
    { printf "%s\n" "$as_me:${as_lineno-$LINENO}: GHC build : $BuildPlatform" >&5
printf "%s\n" "$as_me: GHC build : $BuildPlatform" >&6;}
...
printf "%s\n" "$as_me: WARNING: GHC does not support the Cygwin target at the moment" >&2;}
        { printf "%s\n" "$as_me:${as_lineno-$LINENO}: WARNING: I'm assuming you wanted to build for x86_64-w64-mingw32" >&5
printf "%s\n" "$as_me: WARNING: I'm assuming you wanted to build for x86_64-w64-mingw32" >&2;}
        exit 1
        ;;
...
printf "%s\n" "$as_me: WARNING: Unknown vendor $1" >&2;}
    ...
    checkOS "$BuildOS" "HaskellBuildOs"
...
case "$host_cpu" in
  ...
  *)
    ...
    { printf "%s\n" "$as_me:${as_lineno-$LINENO}: GHC host : $HostPlatform" >&5
printf "%s\n" "$as_me: GHC host : $HostPlatform" >&6;}
...
then :
         ...
         if test x"$TargetArch" = xaarch64; then
            ...
            { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: no, subsections-via-symbols are broken on AArch64/Darwin (GHC 24962)" >&5
printf "%s\n" "no, subsections-via-symbols are broken on AArch64/Darwin (GHC 24962)" >&6; }
         else
...
if test x"${TargetHasSubsectionsViaSymbols}" = x"YES"
then :
...
ac_fn_c_check_header_compile "$LINENO" "sys/cpuset.h" "ac_cv_header_sys_cpuset_h" "#if HAVE_SYS_PARAM_H
# include <sys/param.h>
#endif
...
See \`config.log' for more details" "$LINENO" 5; } ;; #(
  ...
  *) :
    ac_c_undeclared_builtin_options=$ac_cv_c_undeclared_builtin_options ;;
                  ...
                  ac_fn_check_decl "$LINENO" "environ" "ac_cv_have_decl_environ" "
    #include <stdlib.h>
    #include <unistd.h>
...
if test "x$ac_cv_type_char" = xyes
then :

fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking alignment of char" >&5
printf %s "checking alignment of char... " >&6; }
if test ${fp_cv_alignment_char+y}
then :
  printf %s "(cached) " >&6
else $as_nop
  if test "$ac_cv_type_char" = yes; then
  if ac_fn_c_compute_int "$LINENO" "offsetof(struct { char c; char ty; },ty)" "fp_cv_alignment_char"        "$ac_includes_default"
then :
...
See \`config.log' for more details." "$LINENO" 5
  ...
  fp_cv_alignment_char=0
fi
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $fp_cv_alignment_char" >&5
printf "%s\n" "$fp_cv_alignment_char" >&6; }
printf "%s\n" "#define ALIGNMENT_CHAR $fp_cv_alignment_char" >>confdefs.h
...
if test "x$ac_cv_type_double" = xyes
then :

fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking alignment of double" >&5
printf %s "checking alignment of double... " >&6; }
if test ${fp_cv_alignment_double+y}
then :
  printf %s "(cached) " >&6
else $as_nop
  if test "$ac_cv_type_double" = yes; then
  if ac_fn_c_compute_int "$LINENO" "offsetof(struct { char c; double ty; },ty)" "fp_cv_alignment_double"        "$ac_includes_default"
then :
...
See \`config.log' for more details." "$LINENO" 5
  ...
  fp_cv_alignment_double=0
fi
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $fp_cv_alignment_double" >&5
printf "%s\n" "$fp_cv_alignment_double" >&6; }
printf "%s\n" "#define ALIGNMENT_DOUBLE $fp_cv_alignment_double" >>confdefs.h
...
if test "x$ac_cv_type_float" = xyes
then :

fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking alignment of float" >&5
printf %s "checking alignment of float... " >&6; }
if test ${fp_cv_alignment_float+y}
then :
  printf %s "(cached) " >&6
else $as_nop
  if test "$ac_cv_type_float" = yes; then
  if ac_fn_c_compute_int "$LINENO" "offsetof(struct { char c; float ty; },ty)" "fp_cv_alignment_float"        "$ac_includes_default"
then :
...
See \`config.log' for more details." "$LINENO" 5
  ...
  fp_cv_alignment_float=0
fi
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $fp_cv_alignment_float" >&5
printf "%s\n" "$fp_cv_alignment_float" >&6; }
printf "%s\n" "#define ALIGNMENT_FLOAT $fp_cv_alignment_float" >>confdefs.h
...
if test "x$ac_cv_type_int" = xyes
then :

fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking alignment of int" >&5
printf %s "checking alignment of int... " >&6; }
if test ${fp_cv_alignment_int+y}
then :
  printf %s "(cached) " >&6
else $as_nop
  if test "$ac_cv_type_int" = yes; then
  if ac_fn_c_compute_int "$LINENO" "offsetof(struct { char c; int ty; },ty)" "fp_cv_alignment_int"        "$ac_includes_default"
then :
...
See \`config.log' for more details." "$LINENO" 5
  ...
  fp_cv_alignment_int=0
fi
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $fp_cv_alignment_int" >&5
printf "%s\n" "$fp_cv_alignment_int" >&6; }
printf "%s\n" "#define ALIGNMENT_INT $fp_cv_alignment_int" >>confdefs.h
...
if test ${fp_cv_alignment_long+y}
then :
  printf %s "(cached) " >&6
else $as_nop
  if test "$ac_cv_type_long" = yes; then
  if ac_fn_c_compute_int "$LINENO" "offsetof(struct { char c; long ty; },ty)" "fp_cv_alignment_long"        "$ac_includes_default"
then :
...
See \`config.log' for more details." "$LINENO" 5
  ...
  fp_cv_alignment_long=0
fi
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $fp_cv_alignment_long" >&5
printf "%s\n" "$fp_cv_alignment_long" >&6; }
printf "%s\n" "#define ALIGNMENT_LONG $fp_cv_alignment_long" >>confdefs.h
...
if test ${fp_cv_alignment_long_long+y}
then :
  printf %s "(cached) " >&6
else $as_nop
  if test "$ac_cv_type_long_long" = yes; then
  if ac_fn_c_compute_int "$LINENO" "offsetof(struct { char c; long long ty; },ty)" "fp_cv_alignment_long_long"        "$ac_includes_default"
then :
...
See \`config.log' for more details." "$LINENO" 5
  ...
  fp_cv_alignment_long_long=0
fi
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $fp_cv_alignment_long_long" >&5
printf "%s\n" "$fp_cv_alignment_long_long" >&6; }
printf "%s\n" "#define ALIGNMENT_LONG_LONG $fp_cv_alignment_long_long" >>confdefs.h
...
if test "x$ac_cv_type_short" = xyes
then :

fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking alignment of short" >&5
printf %s "checking alignment of short... " >&6; }
if test ${fp_cv_alignment_short+y}
then :
  printf %s "(cached) " >&6
else $as_nop
  if test "$ac_cv_type_short" = yes; then
  if ac_fn_c_compute_int "$LINENO" "offsetof(struct { char c; short ty; },ty)" "fp_cv_alignment_short"        "$ac_includes_default"
then :
...
See \`config.log' for more details." "$LINENO" 5
  ...
  fp_cv_alignment_short=0
fi
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $fp_cv_alignment_short" >&5
printf "%s\n" "$fp_cv_alignment_short" >&6; }
printf "%s\n" "#define ALIGNMENT_SHORT $fp_cv_alignment_short" >>confdefs.h
...
then :
  ...
  if ac_fn_c_compute_int "$LINENO" "(long int) (sizeof (unsigned char))" "ac_cv_sizeof_unsigned_char"        "$ac_includes_default"
then :
...
if test "x$ac_cv_type_unsigned_char" = xyes
then :

fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking alignment of unsigned char" >&5
printf %s "checking alignment of unsigned char... " >&6; }
if test ${fp_cv_alignment_unsigned_char+y}
then :
  printf %s "(cached) " >&6
else $as_nop
  if test "$ac_cv_type_unsigned_char" = yes; then
  if ac_fn_c_compute_int "$LINENO" "offsetof(struct { char c; unsigned char ty; },ty)" "fp_cv_alignment_unsigned_char"        "$ac_includes_default"
then :
...
See \`config.log' for more details." "$LINENO" 5
  ...
  fp_cv_alignment_unsigned_char=0
fi
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $fp_cv_alignment_unsigned_char" >&5
printf "%s\n" "$fp_cv_alignment_unsigned_char" >&6; }
printf "%s\n" "#define ALIGNMENT_UNSIGNED_CHAR $fp_cv_alignment_unsigned_char" >>confdefs.h
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking alignment of unsigned int" >&5
printf %s "checking alignment of unsigned int... " >&6; }
if test ${fp_cv_alignment_unsigned_int+y}
then :
  printf %s "(cached) " >&6
else $as_nop
  if test "$ac_cv_type_unsigned_int" = yes; then
  if ac_fn_c_compute_int "$LINENO" "offsetof(struct { char c; unsigned int ty; },ty)" "fp_cv_alignment_unsigned_int"        "$ac_includes_default"
then :
...
See \`config.log' for more details." "$LINENO" 5
  ...
  fp_cv_alignment_unsigned_int=0
fi
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $fp_cv_alignment_unsigned_int" >&5
printf "%s\n" "$fp_cv_alignment_unsigned_int" >&6; }
printf "%s\n" "#define ALIGNMENT_UNSIGNED_INT $fp_cv_alignment_unsigned_int" >>confdefs.h
...
if test "x$ac_cv_type_unsigned_long" = xyes
then :

fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking alignment of unsigned long" >&5
printf %s "checking alignment of unsigned long... " >&6; }
if test ${fp_cv_alignment_unsigned_long+y}
then :
  printf %s "(cached) " >&6
else $as_nop
  if test "$ac_cv_type_unsigned_long" = yes; then
  if ac_fn_c_compute_int "$LINENO" "offsetof(struct { char c; unsigned long ty; },ty)" "fp_cv_alignment_unsigned_long"        "$ac_includes_default"
then :
...
See \`config.log' for more details." "$LINENO" 5
  ...
  fp_cv_alignment_unsigned_long=0
fi
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $fp_cv_alignment_unsigned_long" >&5
printf "%s\n" "$fp_cv_alignment_unsigned_long" >&6; }
printf "%s\n" "#define ALIGNMENT_UNSIGNED_LONG $fp_cv_alignment_unsigned_long" >>confdefs.h
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking alignment of unsigned long long" >&5
printf %s "checking alignment of unsigned long long... " >&6; }
if test ${fp_cv_alignment_unsigned_long_long+y}
then :
  printf %s "(cached) " >&6
else $as_nop
  if test "$ac_cv_type_unsigned_long_long" = yes; then
  if ac_fn_c_compute_int "$LINENO" "offsetof(struct { char c; unsigned long long ty; },ty)" "fp_cv_alignment_unsigned_long_long"        "$ac_includes_default"
then :
...
See \`config.log' for more details." "$LINENO" 5
  ...
  fp_cv_alignment_unsigned_long_long=0
fi
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $fp_cv_alignment_unsigned_long_long" >&5
printf "%s\n" "$fp_cv_alignment_unsigned_long_long" >&6; }
printf "%s\n" "#define ALIGNMENT_UNSIGNED_LONG_LONG $fp_cv_alignment_unsigned_long_long" >>confdefs.h
...
if test "x$ac_cv_type_unsigned_short" = xyes
then :

fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking alignment of unsigned short" >&5
printf %s "checking alignment of unsigned short... " >&6; }
if test ${fp_cv_alignment_unsigned_short+y}
then :
  printf %s "(cached) " >&6
else $as_nop
  if test "$ac_cv_type_unsigned_short" = yes; then
  if ac_fn_c_compute_int "$LINENO" "offsetof(struct { char c; unsigned short ty; },ty)" "fp_cv_alignment_unsigned_short"        "$ac_includes_default"
then :
...
See \`config.log' for more details." "$LINENO" 5
  ...
  fp_cv_alignment_unsigned_short=0
fi
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $fp_cv_alignment_unsigned_short" >&5
printf "%s\n" "$fp_cv_alignment_unsigned_short" >&6; }
printf "%s\n" "#define ALIGNMENT_UNSIGNED_SHORT $fp_cv_alignment_unsigned_short" >>confdefs.h
...
if test ${fp_cv_alignment_void_p+y}
then :
  printf %s "(cached) " >&6
else $as_nop
  if test "$ac_cv_type_void_p" = yes; then
  if ac_fn_c_compute_int "$LINENO" "offsetof(struct { char c; void * ty; },ty)" "fp_cv_alignment_void_p"        "$ac_includes_default"
then :
...
See \`config.log' for more details." "$LINENO" 5
  ...
  fp_cv_alignment_void_p=0
fi
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $fp_cv_alignment_void_p" >&5
printf "%s\n" "$fp_cv_alignment_void_p" >&6; }
printf "%s\n" "#define ALIGNMENT_VOID_P $fp_cv_alignment_void_p" >>confdefs.h
...
if test ${fp_cv_alignment_int8_t+y}
then :
  printf %s "(cached) " >&6
else $as_nop
  if test "$ac_cv_type_int8_t" = yes; then
  if ac_fn_c_compute_int "$LINENO" "offsetof(struct { char c; int8_t ty; },ty)" "fp_cv_alignment_int8_t"        "$ac_includes_default"
then :

else $as_nop
  as_fn_error 77 "cannot compute alignment (int8_t)
See \`config.log' for more details." "$LINENO" 5
fi

else
  fp_cv_alignment_int8_t=0
fi
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $fp_cv_alignment_int8_t" >&5
printf "%s\n" "$fp_cv_alignment_int8_t" >&6; }
printf "%s\n" "#define ALIGNMENT_INT8_T $fp_cv_alignment_int8_t" >>confdefs.h
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking alignment of uint8_t" >&5
printf %s "checking alignment of uint8_t... " >&6; }
if test ${fp_cv_alignment_uint8_t+y}
then :
  printf %s "(cached) " >&6
else $as_nop
  if test "$ac_cv_type_uint8_t" = yes; then
  if ac_fn_c_compute_int "$LINENO" "offsetof(struct { char c; uint8_t ty; },ty)" "fp_cv_alignment_uint8_t"        "$ac_includes_default"
then :

else $as_nop
  as_fn_error 77 "cannot compute alignment (uint8_t)
See \`config.log' for more details." "$LINENO" 5
fi

else
  fp_cv_alignment_uint8_t=0
fi
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $fp_cv_alignment_uint8_t" >&5
printf "%s\n" "$fp_cv_alignment_uint8_t" >&6; }
printf "%s\n" "#define ALIGNMENT_UINT8_T $fp_cv_alignment_uint8_t" >>confdefs.h
...
if test ${fp_cv_alignment_int16_t+y}
then :
  printf %s "(cached) " >&6
else $as_nop
  if test "$ac_cv_type_int16_t" = yes; then
  if ac_fn_c_compute_int "$LINENO" "offsetof(struct { char c; int16_t ty; },ty)" "fp_cv_alignment_int16_t"        "$ac_includes_default"
then :

else $as_nop
  as_fn_error 77 "cannot compute alignment (int16_t)
See \`config.log' for more details." "$LINENO" 5
fi

else
  fp_cv_alignment_int16_t=0
fi
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $fp_cv_alignment_int16_t" >&5
printf "%s\n" "$fp_cv_alignment_int16_t" >&6; }
printf "%s\n" "#define ALIGNMENT_INT16_T $fp_cv_alignment_int16_t" >>confdefs.h
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking alignment of uint16_t" >&5
printf %s "checking alignment of uint16_t... " >&6; }
if test ${fp_cv_alignment_uint16_t+y}
then :
  printf %s "(cached) " >&6
else $as_nop
  if test "$ac_cv_type_uint16_t" = yes; then
  if ac_fn_c_compute_int "$LINENO" "offsetof(struct { char c; uint16_t ty; },ty)" "fp_cv_alignment_uint16_t"        "$ac_includes_default"
then :

else $as_nop
  as_fn_error 77 "cannot compute alignment (uint16_t)
See \`config.log' for more details." "$LINENO" 5
fi

else
  fp_cv_alignment_uint16_t=0
fi
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $fp_cv_alignment_uint16_t" >&5
printf "%s\n" "$fp_cv_alignment_uint16_t" >&6; }
printf "%s\n" "#define ALIGNMENT_UINT16_T $fp_cv_alignment_uint16_t" >>confdefs.h
...
if test ${fp_cv_alignment_int32_t+y}
then :
  printf %s "(cached) " >&6
else $as_nop
  if test "$ac_cv_type_int32_t" = yes; then
  if ac_fn_c_compute_int "$LINENO" "offsetof(struct { char c; int32_t ty; },ty)" "fp_cv_alignment_int32_t"        "$ac_includes_default"
then :

else $as_nop
  as_fn_error 77 "cannot compute alignment (int32_t)
See \`config.log' for more details." "$LINENO" 5
fi

else
  fp_cv_alignment_int32_t=0
fi
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $fp_cv_alignment_int32_t" >&5
printf "%s\n" "$fp_cv_alignment_int32_t" >&6; }
printf "%s\n" "#define ALIGNMENT_INT32_T $fp_cv_alignment_int32_t" >>confdefs.h
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking alignment of uint32_t" >&5
printf %s "checking alignment of uint32_t... " >&6; }
if test ${fp_cv_alignment_uint32_t+y}
then :
  printf %s "(cached) " >&6
else $as_nop
  if test "$ac_cv_type_uint32_t" = yes; then
  if ac_fn_c_compute_int "$LINENO" "offsetof(struct { char c; uint32_t ty; },ty)" "fp_cv_alignment_uint32_t"        "$ac_includes_default"
then :

else $as_nop
  as_fn_error 77 "cannot compute alignment (uint32_t)
See \`config.log' for more details." "$LINENO" 5
fi

else
  fp_cv_alignment_uint32_t=0
fi
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $fp_cv_alignment_uint32_t" >&5
printf "%s\n" "$fp_cv_alignment_uint32_t" >&6; }
printf "%s\n" "#define ALIGNMENT_UINT32_T $fp_cv_alignment_uint32_t" >>confdefs.h
...
if test ${fp_cv_alignment_int64_t+y}
then :
  printf %s "(cached) " >&6
else $as_nop
  if test "$ac_cv_type_int64_t" = yes; then
  if ac_fn_c_compute_int "$LINENO" "offsetof(struct { char c; int64_t ty; },ty)" "fp_cv_alignment_int64_t"        "$ac_includes_default"
then :

else $as_nop
  as_fn_error 77 "cannot compute alignment (int64_t)
See \`config.log' for more details." "$LINENO" 5
fi

else
  fp_cv_alignment_int64_t=0
fi
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $fp_cv_alignment_int64_t" >&5
printf "%s\n" "$fp_cv_alignment_int64_t" >&6; }
printf "%s\n" "#define ALIGNMENT_INT64_T $fp_cv_alignment_int64_t" >>confdefs.h
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking alignment of uint64_t" >&5
printf %s "checking alignment of uint64_t... " >&6; }
if test ${fp_cv_alignment_uint64_t+y}
then :
  printf %s "(cached) " >&6
else $as_nop
  if test "$ac_cv_type_uint64_t" = yes; then
  if ac_fn_c_compute_int "$LINENO" "offsetof(struct { char c; uint64_t ty; },ty)" "fp_cv_alignment_uint64_t"        "$ac_includes_default"
then :

else $as_nop
  as_fn_error 77 "cannot compute alignment (uint64_t)
See \`config.log' for more details." "$LINENO" 5
fi

else
  fp_cv_alignment_uint64_t=0
fi
fi
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $fp_cv_alignment_uint64_t" >&5
printf "%s\n" "$fp_cv_alignment_uint64_t" >&6; }
printf "%s\n" "#define ALIGNMENT_UINT64_T $fp_cv_alignment_uint64_t" >>confdefs.h




{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for WinExec" >&5
printf %s "checking for WinExec... " >&6; }
if test ${fp_cv_func_WinExec+y}
then :
  printf %s "(cached) " >&6
else $as_nop
  cat confdefs.h - <<_ACEOF >conftest.$ac_ext
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $fp_cv_func_WinExec" >&5
printf "%s\n" "$fp_cv_func_WinExec" >&6; }
if test $fp_cv_func_WinExec = yes
then :

printf "%s\n" "#define HAVE_WINEXEC 1" >>confdefs.h

fi

{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for GetModuleFileName" >&5
printf %s "checking for GetModuleFileName... " >&6; }
if test ${fp_cv_func_GetModuleFileName+y}
then :
  printf %s "(cached) " >&6
else $as_nop
  cat confdefs.h - <<_ACEOF >conftest.$ac_ext
...
if ac_fn_c_try_link "$LINENO"
then :
  fp_cv_func_GetModuleFileName=yes
else $as_nop
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $fp_cv_func_GetModuleFileName" >&5
printf "%s\n" "$fp_cv_func_GetModuleFileName" >&6; }
if test $fp_cv_func_GetModuleFileName = yes
then :

printf "%s\n" "#define HAVE_GETMODULEFILENAME 1" >>confdefs.h
...
ac_fn_check_decl "$LINENO" "ctime_r" "ac_cv_have_decl_ctime_r" "#define _POSIX_SOURCE 1
...
printf "%s\n" "#define HAVE_DECL_CTIME_R $ac_have_decl" >>confdefs.h


ac_fn_check_decl "$LINENO" "program_invocation_short_name" "ac_cv_have_decl_program_invocation_short_name" "#define _GNU_SOURCE 1
#include <errno.h>
" "$ac_c_undeclared_builtin_options" "CFLAGS"
...
then :
  printf "%s\n" "#define HAVE_BFD_H 1" >>confdefs.h
      ...
      { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for xmalloc in -liberty" >&5
printf %s "checking for xmalloc in -liberty... " >&6; }
if test ${ac_cv_lib_iberty_xmalloc+y}
then :
  printf %s "(cached) " >&6
else $as_nop
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $ac_cv_lib_iberty_xmalloc" >&5
printf "%s\n" "$ac_cv_lib_iberty_xmalloc" >&6; }
if test "x$ac_cv_lib_iberty_xmalloc" = xyes
then :
...
then :
      ...
      as_fn_error $? "can't use 'bfd' library" "$LINENO" 5
fi
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    conftest$ac_exeext conftest.$ac_ext
  LIBS="$save_LIBS"
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether float word order is big endian" >&5
printf %s "checking whether float word order is big endian... " >&6; }
if test ${fptools_cv_float_word_order_bigendian+y}
then :
  printf %s "(cached) " >&6
else $as_nop
  cat confdefs.h - <<_ACEOF >conftest.$ac_ext
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $fptools_cv_float_word_order_bigendian" >&5
printf "%s\n" "$fptools_cv_float_word_order_bigendian" >&6; }
...
printf "%s\n" "#define LEADING_UNDERSCORE 1" >>confdefs.h
    ...
    { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether __attribute__((visibility(\"hidden\"))) is supported" >&5
printf %s "checking whether __attribute__((visibility(\"hidden\"))) is supported... " >&6; }
...
printf "%s\n" "#define HAS_VISIBILITY_HIDDEN 1" >>confdefs.h
...
printf "%s\n" "no" >&6; }
    ...
    { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking whether __attribute__((musttail)) is supported" >&5
printf %s "checking whether __attribute__((musttail)) is supported... " >&6; }
...
printf "%s\n" "#define HAS_MUSTTAIL 1" >>confdefs.h
...
then :
  HAVE_timer_create=yes
else $as_nop
  HAVE_timer_create=no
fi
...
printf "%s\n" "#define USE_TIMER_CREATE 1" >>confdefs.h

  else
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for a working timer_create(CLOCK_REALTIME)" >&5
printf %s "checking for a working timer_create(CLOCK_REALTIME)... " >&6; }
if test ${fptools_cv_timer_create_works+y}
then :
  printf %s "(cached) " >&6
else $as_nop
  if test "$cross_compiling" = yes
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $fptools_cv_timer_create_works" >&5
printf "%s\n" "$fptools_cv_timer_create_works" >&6; }
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for printf\$LDBLStub" >&5
printf %s "checking for printf\$LDBLStub... " >&6; }
cat confdefs.h - <<_ACEOF >conftest.$ac_ext
/* end confdefs.h.  */
...
printf "%s\n" "#define HAVE_PRINTF_LDBLSTUB 1" >>confdefs.h
...
printf "%s\n" "#define HAVE_PRINTF_LDBLSTUB 0" >>confdefs.h
...
then :
  printf "%s\n" "#define HAVE_PTHREAD_NP_H 1" >>confdefs.h
      ...
      { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for pthread_setname_np in -lpthread" >&5
printf %s "checking for pthread_setname_np in -lpthread... " >&6; }
if test ${ac_cv_lib_pthread_pthread_setname_np+y}
then :
  printf %s "(cached) " >&6
else $as_nop
  ac_check_lib_save_LIBS=$LIBS
...
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: $ac_cv_lib_pthread_pthread_setname_np" >&5
printf "%s\n" "$ac_cv_lib_pthread_pthread_setname_np" >&6; }
if test "x$ac_cv_lib_pthread_pthread_setname_np" = xyes
then :
  ...
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for pthread_setname_np (Darwin)" >&5
printf %s "checking for pthread_setname_np (Darwin)... " >&6; }
  cat confdefs.h - <<_ACEOF >conftest.$ac_ext
/* end confdefs.h.  */
...
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    ...
    { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for pthread_setname_np (glibc)" >&5
printf %s "checking for pthread_setname_np (glibc)... " >&6; }
  cat confdefs.h - <<_ACEOF >conftest.$ac_ext
/* end confdefs.h.  */
...
rm -f core conftest.err conftest.$ac_objext conftest.beam \
    ...
    { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking for pthread_setname_np (NetBSD)" >&5
printf %s "checking for pthread_setname_np (NetBSD)... " >&6; }
  cat confdefs.h - <<_ACEOF >conftest.$ac_ext
/* end confdefs.h.  */
...
printf %s "checking for __thread support... " >&6; }
cat confdefs.h - <<_ACEOF >conftest.$ac_ext
/* end confdefs.h.  */
 __thread int tester = 0;
_ACEOF
if ac_fn_c_try_compile "$LINENO"
then :
...
printf "%s\n" "#define CC_SUPPORTS_TLS 1" >>confdefs.h
...
printf "%s\n" "#define CC_SUPPORTS_TLS 0" >>confdefs.h
...
# Check whether --enable-large-address-space was given.
if test ${enable_large_address_space+y}
then :
  enableval=$enable_large_address_space; EnableLargeAddressSpace=$enableval
else $as_nop
...
if test "$ac_cv_sizeof_void_p" -eq 8 ; then
    if test "x$EnableLargeAddressSpace" = "xyes" ; then
        if test "$ghc_host_os" = "darwin" ; then
            ...
            ac_fn_check_decl "$LINENO" "MAP_NORESERVE" "ac_cv_have_decl_MAP_NORESERVE" "
                #include <unistd.h>
                #include <sys/types.h>
                #include <sys/mman.h>
                #include <fcntl.h>
...
" "$ac_c_undeclared_builtin_options" "CFLAGS"
if test "x$ac_cv_have_decl_MAP_NORESERVE" = xyes
then :
  ac_have_decl=1
else $as_nop
  ac_have_decl=0
fi
printf "%s\n" "#define HAVE_DECL_MAP_NORESERVE $ac_have_decl" >>confdefs.h
ac_fn_check_decl "$LINENO" "MADV_FREE" "ac_cv_have_decl_MADV_FREE" "
                #include <unistd.h>
                #include <sys/types.h>
                #include <sys/mman.h>
                #include <fcntl.h>
...
" "$ac_c_undeclared_builtin_options" "CFLAGS"
if test "x$ac_cv_have_decl_MADV_FREE" = xyes
then :
  ac_have_decl=1
else $as_nop
  ac_have_decl=0
fi
printf "%s\n" "#define HAVE_DECL_MADV_FREE $ac_have_decl" >>confdefs.h
ac_fn_check_decl "$LINENO" "MADV_DONTNEED" "ac_cv_have_decl_MADV_DONTNEED" "
                #include <unistd.h>
                #include <sys/types.h>
                #include <sys/mman.h>
                #include <fcntl.h>
...
" "$ac_c_undeclared_builtin_options" "CFLAGS"
if test "x$ac_cv_have_decl_MADV_DONTNEED" = xyes
then :
  ac_have_decl=1
else $as_nop
  ac_have_decl=0
fi
printf "%s\n" "#define HAVE_DECL_MADV_DONTNEED $ac_have_decl" >>confdefs.h
...
printf "%s\n" "#define USE_LARGE_ADDRESS_SPACE 1" >>confdefs.h
...
printf "%s\n" "#define RTS_LINKER_USE_MMAP $RtsLinkerUseMmap" >>confdefs.h



    ac_fn_c_check_func "$LINENO" "__aarch64_ldadd1_acq" "ac_cv_func___aarch64_ldadd1_acq"
if test "x$ac_cv_func___aarch64_ldadd1_acq" = xyes
then :

printf "%s\n" "#define HAVE_ARM_OUTLINE_ATOMICS 1" >>confdefs.h
...
printf "%s\n" "#define HAVE_LIBZSTD $CABAL_FLAG_libzstd" >>confdefs.h



printf "%s\n" "#define STATIC_LIBZSTD $CABAL_FLAG_static_libzstd" >>confdefs.h



printf "%s\n" "#define USE_LIBDW $CABAL_FLAG_libdw" >>confdefs.h



printf "%s\n" "#define HAVE_LIBNUMA $CABAL_FLAG_libnuma" >>confdefs.h

if test "$CABAL_FLAG_libnuma" = 1
then :
  ac_fn_c_check_header_compile "$LINENO" "numa.h" "ac_cv_header_numa_h" "$ac_includes_default"
if test "x$ac_cv_header_numa_h" = xyes
then :
  printf "%s\n" "#define HAVE_NUMA_H 1" >>confdefs.h

fi
ac_fn_c_check_header_compile "$LINENO" "numaif.h" "ac_cv_header_numaif_h" "$ac_includes_default"
if test "x$ac_cv_header_numaif_h" = xyes
then :
  printf "%s\n" "#define HAVE_NUMAIF_H 1" >>confdefs.h
...
then :
  ...
  as_fn_error $? "sys/select.h required by select I/O manager" "$LINENO" 5
fi

   fi
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking if the the select I/O manager should be built" >&5
printf %s "checking if the the select I/O manager should be built... " >&6; }
  if test "${EnableIOManagerSelect}" = "YES"; then
    { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: yes" >&5
printf "%s\n" "yes" >&6; }

printf "%s\n" "#define IOMGR_BUILD_SELECT 1" >>confdefs.h
...
printf "%s\n" "no" >&6; }
  ...
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking if the the mio I/O manager should be built" >&5
printf %s "checking if the the mio I/O manager should be built... " >&6; }
  if test "${EnableIOManagerMIO}" = "YES"; then
    { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: yes" >&5
printf "%s\n" "yes" >&6; }

printf "%s\n" "#define IOMGR_BUILD_MIO 1" >>confdefs.h
...
printf "%s\n" "no" >&6; }
  ...
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking if the the win32-legacy I/O manager should be built" >&5
printf %s "checking if the the win32-legacy I/O manager should be built... " >&6; }
  if test "${EnableIOManagerWin32Legacy}" = "YES"; then
    { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: yes" >&5
printf "%s\n" "yes" >&6; }

printf "%s\n" "#define IOMGR_BUILD_WIN32_LEGACY 1" >>confdefs.h
...
printf "%s\n" "no" >&6; }
  ...
  { printf "%s\n" "$as_me:${as_lineno-$LINENO}: checking if the the winio I/O manager should be built" >&5
printf %s "checking if the the winio I/O manager should be built... " >&6; }
  if test "${EnableIOManagerWinIO}" = "YES"; then
    { printf "%s\n" "$as_me:${as_lineno-$LINENO}: result: yes" >&5
printf "%s\n" "yes" >&6; }

printf "%s\n" "#define IOMGR_BUILD_WINIO 1" >>confdefs.h
...
if test "$HostOS" = "mingw32"; then
  ...
  if test "${EnableIOManagerWinIO}" = "YES"; then
    IOManagerNonThreadedDefault=winio
  fi


  if test "${EnableIOManagerWin32Legacy}" = "YES"; then
    IOManagerNonThreadedDefault=win32-legacy
  fi


  if test "${EnableIOManagerWinIO}" = "YES"; then
    IOManagerThreadedDefault=winio
  fi


  if test "${EnableIOManagerMIO}" = "YES"; then
    IOManagerThreadedDefault=mio
  fi

else

  if test "${EnableIOManagerSelect}" = "YES"; then
    IOManagerNonThreadedDefault=select
  fi
  ...
  if test "${IOManagerNonThreadedDefault}" = ""; then
    as_fn_error $? "no suitable I/O manager enabled for the non-threaded RTS" "$LINENO" 5
  fi


  if test "${IOManagerThreadedDefault}" = ""; then
    as_fn_error $? "no suitable I/O manager enabled for the threaded RTS" "$LINENO" 5
  fi


{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: default I/O manager for the non-threaded RTS: ${IOManagerNonThreadedDefault}" >&5
printf "%s\n" "$as_me: default I/O manager for the non-threaded RTS: ${IOManagerNonThreadedDefault}" >&6;}
{ printf "%s\n" "$as_me:${as_lineno-$LINENO}: default I/O manager for the threaded RTS: ${IOManagerThreadedDefault}" >&5
printf "%s\n" "$as_me: default I/O manager for the threaded RTS: ${IOManagerThreadedDefault}" >&6;}


  if test "${IOManagerNonThreadedDefault}" = "select"; then

printf "%s\n" "#define IOMGR_DEFAULT_NON_THREADED_SELECT 1" >>confdefs.h

  fi



  if test "${IOManagerNonThreadedDefault}" = "winio"; then

printf "%s\n" "#define IOMGR_DEFAULT_NON_THREADED_WINIO 1" >>confdefs.h

  fi



  if test "${IOManagerNonThreadedDefault}" = "win32-legacy"; then

printf "%s\n" "#define IOMGR_DEFAULT_NON_THREADED_WIN32_LEGACY 1" >>confdefs.h

  fi



  if test "${IOManagerThreadedDefault}" = "mio"; then

printf "%s\n" "#define IOMGR_DEFAULT_THREADED_MIO 1" >>confdefs.h

  fi



  if test "${IOManagerThreadedDefault}" = "winio"; then

printf "%s\n" "#define IOMGR_DEFAULT_THREADED_WINIO 1" >>confdefs.h
...
ac_log="
This file was extended by GHC run-time system $as_me 1.0.2, which was
generated by GNU Autoconf 2.71.  Invocation command line was
...
ac_cs_version="\\
GHC run-time system config.status 1.0.2
...
do
  case $ac_config_target in
    "ghcplatform.h.top") CONFIG_FILES="$CONFIG_FILES ghcplatform.h.top" ;;
    "ghcautoconf.h.autoconf") CONFIG_HEADERS="$CONFIG_HEADERS ghcautoconf.h.autoconf" ;;
...
if test "$CABAL_FLAG_unregisterised" = 1
then :
