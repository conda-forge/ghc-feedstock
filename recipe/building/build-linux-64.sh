...
perl -pi -e 's#(C compiler link flags", "[^"]*)#$1 -Wl,-L$ENV{BUILD_PREFIX}/lib -Wl,-L$ENV{PREFIX}/lib -Wl,-L\$topdir/../../../../lib -Wl,-rpath,\$topdir/../../../../lib#' "${settings_file}"
perl -pi -e 's#(ld flags", "[^"]*)#$1 -L$ENV{BUILD_PREFIX}/lib -L$ENV{PREFIX}/lib -L\$topdir/../../../../lib -rpath \$topdir/../../../../lib#' "${settings_file}"
...
perl -pi -e 's#(C\+\+ compiler flags", "[^"]*)#$1 -Wno-strict-prototypes#' "${settings_file}"
perl -pi -e 's#(C compiler link flags", "[^"]*)#$1 -Wl,-L$ENV{PREFIX}/lib -Wl,-L\$topdir/../../../../lib -Wl,-rpath,\$topdir/../../../../lib#' "${settings_file}"
perl -pi -e 's#(ld flags", "[^"]*)#$1 -L$ENV{PREFIX}/lib -L\$topdir/../../../../lib -rpath \$topdir/../../../../lib#' "${settings_file}"
...
find "${PREFIX}"/lib/ghc-"${PKG_VERSION}" -name "*.a" -exec rm {} \;

# CRITICAL: Create .so symlinks for dynamic linking
# GHC generates library files like libHSbase-4.18.3.0-ghc9.6.7.so
# But linker flags use -lHSbase-4.18.3.0 which looks for libHSbase-4.18.3.0.so
# We need symlinks from short names to versioned names
echo "Creating .so symlinks for dynamic linking..."
find "${PREFIX}"/lib/ghc-"${PKG_VERSION}"/lib/x86_64-linux-ghc-"${PKG_VERSION}" -name "*.so" -type f | while read -r fullpath; do
  dir=$(dirname "$fullpath")
  fullname=$(basename "$fullpath")
  # Remove -ghc9.6.7 suffix to get short name
  shortname=${fullname%-ghc${PKG_VERSION}.so}.so
  if [ "$shortname" != "$fullname" ]; then
    echo "  Creating symlink: $shortname -> $fullname"
    ln -sf "$fullname" "$dir/$shortname"
  fi
done
