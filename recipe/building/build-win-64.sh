...
if [[ -f "${settings_file}" ]]; then
  ...
  # CRITICAL: --subsystem console for console entry point (linker directive - TWO SEPARATE FLAGS!)
...
export LIBS="-Wl,--subsystem -Wl,console -lmoldname -lmingwex -lmingw32 ${CHKSTK_LIB} -lmsvcrt -lkernel32 -ladvapi32"

# CRITICAL: Also add to LDFLAGS with proper linker subsystem flag
# -Wl,--subsystem -Wl,console: Use console entry point (main) instead of GUI (WinMain)
# This is a linker directive, not a compiler flag
export LDFLAGS="${LDFLAGS} -Wl,--subsystem -Wl,console"
