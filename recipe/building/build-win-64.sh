...
if [[ -f "${settings_file}" ]]; then
  ...
  perl -pi -e 's#(compiler link flags",\s*"[^"]*)#$1 -Wl,-L\$topdir/Library/lib -Wl,-L\$topdir#' "${settings_file}"
  perl -pi -e 's#(ld flags",\s*"[^"]*)#$1 -L\$topdir/Library/lib -L\$topdir#' "${settings_file}"
  cat "${settings_file}"
else
