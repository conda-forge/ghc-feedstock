...
(
  ...
  pushd "${_SRC_DIR}"/hadrian
    ...
    # Use --enable-executable-static to build hadrian as a fully static executable
    # This avoids DLL dependency issues when executing from bash
    timeout 600 "${CABAL}" v2-build -j --with-ld="${LD}" --extra-lib-dirs="${_BUILD_PREFIX}/Library/lib/gcc/x86_64-w64-mingw32/15.2.0" --ghc-options="-optl-lgcc -optl-Wl,--image-base -optl-Wl,0x140000000" --enable-executable-static hadrian 2>&1 | tee "${_SRC_DIR}"/cabal-build.log
    ...
    if [[ $_cabal_exit_code -ne 0 ]]; then
      ...
      for pkg in file-io clock js-dgtable heaps js-flot js-jquery os-string splitmix primitive utf8-string directory random; do
        ...
        timeout 60 "${CABAL}" v2-build --with-ld="${LD}" --extra-lib-dirs="${_BUILD_PREFIX}/Library/lib/gcc/x86_64-w64-mingw32/15.2.0" --ghc-options="-optl-lgcc -optl-Wl,--image-base -optl-Wl,0x140000000" --enable-executable-static "$pkg" 2>&1 | tee "${_SRC_DIR}/package-${pkg}.log"
        pkg_exit=$?
      ...
      timeout 300 "${CABAL}" v2-build -j -v3 --with-ld="${LD}" --extra-lib-dirs="${_BUILD_PREFIX}/Library/lib/gcc/x86_64-w64-mingw32/15.2.0" --ghc-options="-optl-lgcc -optl-Wl,--image-base -optl-Wl,0x140000000" --enable-executable-static hadrian 2>&1 | tee "${_SRC_DIR}"/cabal-verbose.log
      exit 1
    else
