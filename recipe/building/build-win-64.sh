...
# CRITICAL: Use Unix-style path to avoid conda's %BUILD_PREFIX% placeholder
...
# CRITICAL: Expand ALL conda environment variables BEFORE configure
# Conda activation scripts set variables with %BUILD_PREFIX%, %PREFIX%, %SRC_DIR%
# GHC's configure script picks these up and writes them into config files
# This causes Hadrian to look for paths like "%BUILD_PREFIX%/ghc-bootstrap/mingw/bin/ld.exe"
echo "=== Expanding ALL conda environment variables ==="

# Export expanded toolchain paths - these override conda's unexpanded values
export AR="${_BUILD_PREFIX}/Library/bin/x86_64-w64-mingw32-ar.exe"
export LD="${_BUILD_PREFIX}/Library/bin/x86_64-w64-mingw32-ld.exe"
...
# Expand GCC/G++/CPP if they contain unexpanded variables
if [[ "${CC}" == *"%"* ]]; then
  export CC="x86_64-w64-mingw32-gcc"
fi
if [[ "${CXX}" == *"%"* ]]; then
...
# Expand GCC toolchain wrappers
export GCC="${_BUILD_PREFIX}/Library/bin/x86_64-w64-mingw32-gcc.exe"
export GXX="${_BUILD_PREFIX}/Library/bin/x86_64-w64-mingw32-g++.exe"
export GCC_AR="${_BUILD_PREFIX}/Library/bin/x86_64-w64-mingw32-gcc-ar.exe"
export GCC_NM="${_BUILD_PREFIX}/Library/bin/x86_64-w64-mingw32-gcc-nm.exe"
export GCC_RANLIB="${_BUILD_PREFIX}/Library/bin/x86_64-w64-mingw32-gcc-ranlib.exe"

echo "Expanded toolchain environment variables:"
echo "  AR=${AR}"
echo "  LD=${LD}"
echo "  NM=${NM}"
echo "  RANLIB=${RANLIB}"
...
if [[ -f "${settings_file}" ]]; then
  ...
  perl -pi -e 's#(compiler link flags",\s*"[^"]*)#$1 -Wl,-L\$topdir -Wl,rpath,\$topdir#' "${settings_file}"
  perl -pi -e 's#(ld flags",\s*"[^"]*)#$1 -L\$topdir -rpath \$topdir#' "${settings_file}"
  cat "${settings_file}"
else
