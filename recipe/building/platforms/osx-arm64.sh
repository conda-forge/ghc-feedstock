...
# - Creates osx64_env conda environment for x86_64 bootstrap
...
platform_setup_environment() {
  ...
  # Create environment for x86_64 cross-compilation libraries
  echo "  Creating osx64_env for cross-compilation..."
  conda create -y \
    -n osx64_env \
    --platform osx-64 \
    -c conda-forge \
  ...
  export osx64_env=$(conda info --envs | grep osx64_env | awk '{print $2}')
  ghc_path="${osx64_env}/ghc-bootstrap/bin"

  if [[ -z "${osx64_env}" ]]; then
    echo "ERROR: Failed to find osx64_env"
    conda info --envs
    exit 1
  fi
  echo "  osx64 environment: ${osx64_env}"

  export GHC="${ghc_path}/ghc"
  "${ghc_path}/ghc-pkg" recache

  export CABAL="${osx64_env}/bin/cabal"
...
platform_post_configure_ghc() {
  ...
  # Set system-ar to llvm-ar for stage0
  perl -pi -e "s#(system-ar\s*?=\s).*#\$1${AR_STAGE0}#" "${settings_file}"

  # Set stage0 compiler flags for host targeting
  ...
  local bootstrap_settings="${osx64_env}/ghc-bootstrap/lib/ghc-${PKG_VERSION}/lib/settings"
