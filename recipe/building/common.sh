...
update_settings_link_flags() {
  ...
  if [[ "${target_platform}" == "linux-"* ]]; then
    ...
    perl -pi -e 's#(C compiler flags", "[^"]*)#$1 -fno-lto -Wno-strict-prototypes#' "${settings_file}"
    perl -pi -e 's#(C\+\+ compiler flags", "[^"]*)#$1 -fno-lto -Wno-strict-prototypes#' "${settings_file}"
...
update_installed_settings() {
  ...
  if [[ "${target_platform}" == "linux-"* ]]; then
    ...
    perl -pi -e "s#(C compiler supports -no-pie\", \")[^\"]*#\$1YES#" "${settings_file}"

  elif [[ "${target_platform}" == "osx-"* ]]; then
    echo "DBG: Update ${target_platform} ${settings_file}"
  ...
  # Remove absolute BUILD_PREFIX and PREFIX paths from tool commands
  perl -pi -e "s#(${BUILD_PREFIX}/bin/|${PREFIX}/bin/)([a-zA-Z0-9_-]+)#\$2#g" "${settings_file}"

  # Remove absolute BUILD_PREFIX and PREFIX library paths (but preserve $topdir relative paths)
  # Only match flags followed by actual absolute paths starting with / or $BUILD_PREFIX or $PREFIX
  perl -pi -e "s#-Wl,-L${BUILD_PREFIX}/lib(\s|\"|\))#\$1#g" "${settings_file}"
  perl -pi -e "s#-Wl,-L${PREFIX}/lib(\s|\"|\))#\$1#g" "${settings_file}"
  perl -pi -e "s#-Wl,-rpath,${BUILD_PREFIX}/lib(\s|\"|\))#\$1#g" "${settings_file}"
  perl -pi -e "s#-Wl,-rpath,${PREFIX}/lib(\s|\"|\))#\$1#g" "${settings_file}"
  perl -pi -e "s#-L${BUILD_PREFIX}/lib(\s|\"|\))#\$1#g" "${settings_file}"
  perl -pi -e "s#-L${PREFIX}/lib(\s|\"|\))#\$1#g" "${settings_file}"
  perl -pi -e "s#-rpath ${BUILD_PREFIX}/lib(\s|\"|\))#\$1#g" "${settings_file}"
  perl -pi -e "s#-rpath ${PREFIX}/lib(\s|\"|\))#\$1#g" "${settings_file}"

  perl -pi -e "s#\"[/\w]*?(ar|clang|clang\+\+|ld|ranlib|llc|objdump|opt)\"#\"${toolchain}-\$1\"#" "${settings_file}"

  # Deduplicate all flags after modifications
  deduplicate_settings_flags "${settings_file}"
  cat "${settings_file}"
}
...
# Deduplicate flags in settings file for both space-separated and value lists
#
# This function removes duplicate flags from settings file entries while preserving order.
# Handles both:
#   - Space-separated flags: "-flag1 -flag2 -flag1" -> "-flag1 -flag2"
#   - Comma-separated values: "val1, val2, val1" -> "val1, val2"
#
# Usage:
#   deduplicate_settings_flags "${settings_file}"
#
...
#
# Examples:
#   Input:  ,("C compiler link flags", "-Wl,-L$PREFIX/lib -Wl,-rpath,$PREFIX/lib -Wl,-L$PREFIX/lib")
#   Output: ,("C compiler link flags", "-Wl,-L$PREFIX/lib -Wl,-rpath,$PREFIX/lib")
#
#   Input:  ,("ld flags", " -L$PREFIX/lib -liconv -L$PREFIX/lib -liconv")
#   Output: ,("ld flags", " -L$PREFIX/lib -liconv")
#
deduplicate_settings_flags() {
  local settings_file="$1"
  ...
  local tmp_file="${settings_file}.dedup.tmp"

  # Process each line with Perl to deduplicate flags
  perl -pe '
    # Process lines that look like settings entries: ,("key", "value")
    if (/^\s*,\("([^"]+)",\s*"([^"]*)"\)/) {
      my $key = $1;
      my $value = $2;
      my $original = $value;

      # Only process flag-related entries (avoid corrupting version strings, paths, etc.)
      if ($key =~ /(flags|command|supports|options)/i) {
        # Split on spaces, deduplicate while preserving order, rejoin
